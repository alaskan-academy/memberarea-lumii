  ...
  return (
    <>
      <div
        ...
        className="lumii-card overflow-hidden flex flex-col cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-lumii-coral"
      >
        {/* Thumbnail */}
        <div className="aspect-video bg-muted relative overflow-hidden group/thumb">
          {course.thumbnail_url ? (
            ...
            <div className="w-full h-full flex items-center justify-center bg-lumii-coral/10 text-3xl">
              🎨
            </div>
          )}
          {isComplete && (
            <div className="absolute top-2 right-2 bg-lumii-green text-white text-[10px] font-bold px-2 py-0.5 rounded-full">
              Concluído ✓
            </div>
          )}
          ...
          <div className="absolute inset-0 bg-black/20 opacity-0 group-hover/thumb:opacity-100 transition-opacity flex items-center justify-center">
            <span className="bg-white/90 text-lumii-coral text-xs font-semibold px-3 py-1.5 rounded-full">
              Ver conteúdo
            </span>
          </div>
        </div>
        ...
        <div className="p-4 flex flex-col gap-3 flex-1">
          <h3 className="font-semibold text-sm line-clamp-2 min-h-[2.5rem] leading-5 group-hover:text-lumii-coral transition-colors">
            {course.title}
          </h3>
          ...
          {progress.total > 0 && (
            <div className="space-y-1">
              <div className="flex justify-between items-center text-[11px] text-muted-foreground">
                ...
                <span className={cn("font-semibold", isComplete ? "text-lumii-green" : "text-lumii-coral")}>
                  {progress.percentage}%
                </span>
              </div>
