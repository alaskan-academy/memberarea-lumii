...
const TYPE_CONFIG = {
  forum_post:      { icon: MessageSquare,  label: "Post no fórum",         color: "#f6614f", bg: "bg-lumii-coral/10",  filter: "forum"       as FilterKey },
  forum_comment:   { icon: MessageCircle,  label: "Comentário no fórum",   color: "#f6614f", bg: "bg-lumii-coral/10",  filter: "comments"    as FilterKey },
  suggestion:      { icon: Store,          label: "Sugestão de fornecedor", color: "#eebc3e", bg: "bg-lumii-yellow/15",  filter: "suggestions" as FilterKey },
  lesson_completed:{ icon: CheckCircle2,   label: "Aula concluída",         color: "#71c69a", bg: "bg-lumii-green/10",  filter: "lessons"     as FilterKey },
  insp_like:       { icon: Heart,          label: "Curtiu inspiração",      color: "#f87171", bg: "bg-red-50",        filter: "inspiracoes" as FilterKey },
  insp_bookmark:   { icon: Bookmark,       label: "Salvou inspiração",      color: "#f6614f", bg: "bg-lumii-coral/10",  filter: "inspiracoes" as FilterKey },
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        {forumPosts > 0 && (
          <span className="px-3 py-1 rounded-full text-xs font-semibold bg-lumii-coral/10 text-lumii-coral">
        ...
        {lessonsCompleted > 0 && (
          <span className="px-3 py-1 rounded-full text-xs font-semibold bg-lumii-green/10 text-[#5bb577]">
            {lessonsCompleted} aula{lessonsCompleted !== 1 ? "s" : ""} concluída{lessonsCompleted !== 1 ? "s" : ""}
          </span>
        )}
        {suggestions > 0 && (
          <span className="px-3 py-1 rounded-full text-xs font-semibold bg-lumii-yellow/15 text-yellow-700">
            ...
            <button
              ...
              className={cn(
                ...
                filter === f.key
                  ...
                  : "bg-white text-muted-foreground border-border hover:border-lumii-coral/40 hover:text-foreground"
              )}
            >
              {f.label}
