...
export default function ForumModerationClient({ posts: initialPosts }: Props) {
  ...
  const PostCard = ({ post }: { post: ForumPostRow }) => (
    <div key={post.id} className={cn(
      ...
      !post.approved ? "border-lumii-yellow/40 bg-lumii-yellow/5" : "border-border/60"
    )}>
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            <span className="text-xs font-semibold text-lumii-coral bg-lumii-coral/10 px-2 py-0.5 rounded-full">
              {post.forum_title}
            </span>
            {!post.approved && (
              <span className="flex items-center gap-1 text-xs font-semibold text-[#b8900d] bg-lumii-yellow/15 px-2 py-0.5 rounded-full">
                <Clock className="w-3 h-3" /> Pendente
              </span>
            )}
            {post.pinned && (
              <span className="text-xs font-semibold text-lumii-green bg-lumii-green/10 px-2 py-0.5 rounded-full">Fixado</span>
            )}
          </div>
          ...
          {post.attachment_url && (
            <a href={post.attachment_url} target="_blank" rel="noopener noreferrer"
              className="inline-flex items-center gap-1 text-[10px] text-lumii-coral hover:underline mb-2">
          ...
          {!post.approved && (
            <div className="flex gap-1 mb-1">
              <button
                ...
                title="Aprovar post"
                className="flex items-center gap-1 px-2 py-1 rounded-lg text-xs font-semibold text-white bg-lumii-green hover:opacity-90 transition-opacity"
              >
          ...
          <div className="flex gap-1">
            {post.forum_slug && (
              <Link
                ...
                target="_blank"
                title="Ver no fórum"
                className="p-1.5 rounded-lg text-muted-foreground hover:text-lumii-coral hover:bg-lumii-coral/10 transition-colors"
              >
            ...
            {post.approved && (
              <button
                ...
                className="p-1.5 rounded-lg text-muted-foreground hover:text-lumii-yellow hover:bg-lumii-yellow/10 transition-colors"
              >
