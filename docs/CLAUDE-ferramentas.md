...
- **"Meus Alunos"** (`/ferramentas/meus-alunos`, ex-`plano-apoio-aluno` / ex-"Plano de Apoio" — virou o hub do Bloco 1 "aluno é o centro" ao ganhar o Diário) — professores. Hub lista alunos/turmas; a **ficha do aluno tem abas**: **Diário de bordo** + **Plano de apoio** + **Metas** (socioemocionais, `student_goals` + `student_goal_checkins`) + **Portfólio** (trabalhos do aluno c/ foto opcional, `student_portfolio` + bucket privado `portfolio`) + **Relatório** (consolida diário+plano+rubricas num relatório de conselho/reunião; gated `completo` via `tierAtLeast` — aluna vê teaser `RelatorioLocked`, admin/completo vê `RelatorioAluno` com copiar/imprimir; read-only, sem tabela nova). O hub tem ainda o **Mapa da turma** (`/ferramentas/meus-alunos/mapa`, gated `completo`): dashboard que agrega por aluno os sinais de diário+plano+rubricas (conquistas/atenção/plano ativo/avaliações), ordena por "atenção primeiro", e cada linha leva ao relatório do aluno — read-only (`src/lib/ferramentas/turma-mapa.ts`). A rota antiga `plano-apoio-aluno/*` redireciona para cá (catch-all `[[...rest]]`).
...
- **"Planejamento"** (`/ferramentas/planejamento`, Bloco 2) — professores (pago). **Hub com abas** (`?aba=`): **Biblioteca** de planos/atividades (`lesson_resources`) + **Rubricas** (`rubrics` + `rubric_scores`) — ambas tier aluna — + **Planejador BNCC** (`year_plans`, tier **completo**: aluna vê teaser; as actions checam `completo` via `requireCompleto()`, 1º gate de tier no nível de action).
...
student_goals               ← Metas socioemocionais por aluno (area, meta, status) (migration 20260908_student_goals.sql)
student_goal_checkins       ← Progresso das metas (avancou/estavel/recuou) — SEM teacher_id, RLS via subquery à meta pai
student_portfolio           ← Portfólio do aluno (titulo, descricao, anexo_path) (migration 20260909_student_portfolio.sql)
year_plans                  ← Planejador BNCC (ano, bimestre, componente, bncc_codes[], conteudo) — tier completo (migration 20260909_year_plans.sql)
```

`student_portfolio` guarda a FOTO num **bucket privado `portfolio`** (nunca público — dado de criança). Path `{teacher_id}/{student_id}/{uuid}.webp`; RLS do bucket = `(storage.foldername(name))[1] = auth.uid()` (padrão de `005_certificates.sql`). Upload é server action (`portfolio/actions.ts`): comprime no client (canvas→webp), o servidor valida MIME/tamanho + `prepareImageForUpload` + sobe via service client; leitura assina URL curta (service). Excluir remove o objeto do storage.
